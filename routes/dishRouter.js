...
.post(authenticate.verifyUser,(req,res,next) => {
    ...
    .then((dish) => {
        if (dish != null) {
            // agregando el id del usuario al comentario del objeto User
            // devuelto por el athenticate.verifyUser
            req.body.author = req.user._id;
            dish.comments.push(req.body);
            dish.save()
            .then((dish) => {
                Dishes.findById(dish.id)
                    // populate para enviar la respuesta
                    .populate('comments.author')
                    .then((user) => {
                        res.statusCode = 200;
                        res.setHeader('Content-Type','aplicattion/json');
...
.put(authenticate.verifyUser,(req,res,next) => {
    ...
    .then((dish) => {
        // cheking
        if (dish != null && dish.comments.id(req.params.commentId) != null ) {
            // Cheking the user to see if he is the author of the comment 
            if ( req.user._id.equals( dish.comments.id(req.params.commentId).author._id )) {

                if (req.body.raiting) {
                    dish.comments.id(req.params.commentId).raiting = req.body.raiting;
                }
                if (req.body.comment) {
